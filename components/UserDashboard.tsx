...
import { calculateDistance, getDeviceFingerprint, getEgyptTime, getRealNetworkTime, checkSecurityStatus, getNativeSecureLocation, ensureLocationPermission } from '../utils';
...
const UserDashboard: React.FC<UserDashboardProps> = ({ 
  ...
  useEffect(() => {
    ...
    // Ask for the Android runtime permission first, then start watching.
  ...
  const handleAttendance = async (type: 'check-in' | 'check-out') => {
    ...
    // فحص أمني أولي: وضع المطورين / تطبيقات تزييف الموقع / المحاكيات
    try {
    ...
    // نطلب الموقع دائماً بشكل جديد عند التسجيل (لا نعتمد على موقع الخلفية)،
    // لأن العلم الرسمي isMock يجب أن يُقرأ من قراءة حديثة.
    let lat = 0;
    let lng = 0;
    let accuracy = 0;
    ...
    if (!hasPermission) {
        setIsVerifying(false);
        const msg = 'التطبيق يحتاج إذن الوصول للموقع الجغرافي لتسجيل الحضور. يرجى تفعيله من إعدادات الهاتف.';
        setStatus({ type: 'error', msg });
        logAction(`فشل تسجيل ${type === 'check-in' ? 'حضور' : 'انصراف'}`, `السبب: ${msg}`);
        return;
    }

    // (أ) المسار الأصلي (APK): قراءة أصلية تحمل علم isMock من النظام،
    //     وهو العلم الوحيد الذي لا يستطيع تطبيق Fake GPS تزويره.
    const nativeLoc = await getNativeSecureLocation();

    if (nativeLoc) {
        lat = nativeLoc.latitude;
        lng = nativeLoc.longitude;
        accuracy = nativeLoc.accuracy;
        isMockDetected = nativeLoc.isMock;
    } else {
        // (ب) متصفح الويب أو تعذّر المسار الأصلي
        try {
            ...
            setIsVerifying(false);
            const msg = 'تعذر تحديد الموقع الحالي بدقة. تأكد من تفعيل GPS والسماح بالوصول للموقع ثم المحاولة مرة أخرى.';
            setStatus({ type: 'error', msg });
            logAction(`فشل تسجيل ${type === 'check-in' ? 'حضور' : 'انصراف'}`, `السبب: ${msg}`);
            return;
        }
    ...
    setLiveLocation({ lat, lng, accuracy, timestamp: Date.now() });

    // فحص أمني ثانٍ على الإحداثيات نفسها (isMock / 0,0 / دقة ضعيفة)
    const locSecRes = await checkSecurityStatus({
        coords: { latitude: lat, longitude: lng, accuracy, isMock: isMockDetected }
    });
    if (!locSecRes.isAllowed) {
        setIsVerifying(false);
        setStatus({ type: 'error', msg: locSecRes.reason || 'تم حظر التسجيل لأسباب أمنية.' });
        logAction(`فشل تسجيل ${type === 'check-in' ? 'حضور' : 'انصراف'}`, `السبب: الأمان - ${locSecRes.reason} | الإحداثيات: ${lat}, ${lng}`);
        return;
    }
